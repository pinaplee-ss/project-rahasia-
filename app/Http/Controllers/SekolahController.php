...
class SekolahController extends Controller
{
    ...
    {
        $sekolah = Sekolah::with('operator')->get();
    ...
    public function store(Request $request)
    {
        $request -> validate([
            ...
            'foto_sekolah' => 'nullable|image|mimes:jpeg.png.jpg,webp|max:1024',
            'longitude' => 'nullable|numeric',
            'latitude' => 'nullable|numeric',
        ]);

        $data = $request->except(['_token', 'foto_sekolah']);
        $sekolah = new Sekolah();
        $path = $sekolah->fotoProfil($request);

        if ($path) {
            $data ['foto_sekolah'] = $path;
        } else {
            $data ['foto_sekoloah'] = null;
        }
